...
Starting in Spring 2025 an alternative GPT-2 Transformer Project has been offered, replacing the Milestone 2 and Milestone 3 of the original project. The GPT-2 project is done in teams of 3-4 with the goal of building a fully-functional version of the model, diving deep into the transformer architecture. This project, as of Fall 2025, is split into three checkpoints, each with an in-person demo and quiz. These quizzes, as of Fall 2025, are done orally, in a style historically similar to that of ECE 385. The checkpoints are as follows:

- Milestone 1: Implement the "naive" GPT-2 transformer model, including the **unoptimized** encoder layer, attention, layer normalization, GELU, as well as other required kernels.
- Milestone 2: Implement a base set of provided optimizations, including, but not limited to, joint shared memory and register tiling, and the utilization of cuBLAS and tensor cores. Profile the baseline and optimized transformer model and kernels with NSight Systems and Compute. Propose a set of extra optimizations to implement for Milestone 3. Lastly, write a report detailing all the optimizations made, as well as proposed optimizations and the motivation behind them.
- Milestone 3: Implement another set of provided optimizations, as well as the proposed optimizations from Milestone 2. Profile the kernels, and write a report detailing all the optimizations you've added. 
...
- The Quizzes: The lab quizzes tend to be rather difficult, and a good representation of potential midterm problems. 
